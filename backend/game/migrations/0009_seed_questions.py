# Generated by Django 6.0.1 on 2026-08-25 11:09

from django.db import migrations
import hashlib

def seed_questions(apps, schema_editor):
    Questions = apps.get_model("game", "Questions")
    ...
    for level, questions in raw_data.items():
        for item in questions:
            fingerprint = hashlib.sha256(
                f"{level}:{item['question']}".encode("utf-8")
            ).hexdigest()
            correct_idx = item["options"].index(item["answer"])
...
def reverse_seed_questions(apps, schema_editor):
    Questions = apps.get_model("game", "Questions")
    Questions.objects.filter(source="seed").delete()

class Migration(migrations.Migration):

    dependencies = [
        ('game', '0008_questions_questionattempt'),
    ]

    operations = [
        migrations.RunPython(seed_questions, reverse_code=reverse_seed_questions),
    ]
